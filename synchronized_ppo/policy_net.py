    ...
    def __init__(self, name: str, state_size, action_size, temp=0.1):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('policy_net'):
                layer_1 = tf.layers.conv2d(inputs=reshape, filters=8, kernel_size=[7, 7], activation=tf.nn.relu)
                layer_2 = tf.layers.conv2d(inputs=layer_1, filters=16, kernel_size=[7, 7], activation=tf.nn.relu)
                layer_6 = tf.reshape(layer_2, [-1, 4*4*16])
                layer_7 = tf.layers.dense(inputs=layer_6, units=32, activation=tf.nn.relu)
                self.act_probs = tf.layers.dense(inputs=tf.divide(layer_7, temp), units=action_size, activation=tf.nn.softmax)

            with tf.variable_scope('value_net'):
                layer_1 = tf.layers.conv2d(inputs=reshape, filters=8, kernel_size=[7, 7], activation=tf.tanh)
                layer_2 = tf.layers.conv2d(inputs=layer_1, filters=16, kernel_size=[7, 7], activation=tf.tanh)
                layer_6 = tf.reshape(layer_2, [-1, 4*4*16])
                layer_7 = tf.layers.dense(inputs=layer_6, units=32, activation=None, trainable=True)
